(function () {
  ...
  const form = c => `<form class="form-grid">
    ...
    <div class="field"><label>Nome / Razão social *</label><input name="name" required value="${U.esc(c?.name || "")}"></div>
    <div class="field"><label>CPF</label><input name="cpf" value="${U.esc(c?.cpf || "")}"></div>
    <div class="field"><label>CNPJ</label><div class="input-action"><input name="cnpj" value="${U.esc(c?.cnpj || "")}"><button type="button" class="btn small" data-lookup-cnpj>Buscar</button></div></div>
    <div class="field"><label>CEP</label><div class="input-action"><input name="cep" value="${U.esc(c?.cep || "")}"><button type="button" class="btn small" data-lookup-cep>Buscar</button></div></div>
  ...
  function edit(id) {
    ...
    U.modal({ title: c ? "Editar cliente" : "Novo cliente", body: form(c), onOpen: setupLookups, onSave: root => {
  ...
  function field(root,prefix,name){return root.querySelector(`[name=${prefix ? prefix + name[0].toUpperCase() + name.slice(1) : name}]`)}
  function setupLookups(root){
    root.querySelectorAll("[data-lookup-cnpj]").forEach(btn=>btn.onclick=async()=>{const p=btn.dataset.prefix||"",input=field(root,p,"cnpj"),cnpj=input.value.replace(/\D/g,"");if(cnpj.length!==14)return U.toast("Informe um CNPJ com 14 dígitos.","error");btn.disabled=true;btn.textContent="Buscando…";try{const r=await fetch(`https://brasilapi.com.br/api/cnpj/v1/${cnpj}`);if(!r.ok)throw new Error();const x=await r.json(),address=[x.descricao_tipo_de_logradouro,x.logradouro,x.numero,x.complemento,x.bairro,x.municipio,x.uf].filter(Boolean).join(", ");field(root,p,"name").value=x.nome_fantasia||x.razao_social||field(root,p,"name").value;field(root,p,"phone").value=x.ddd_telefone_1||field(root,p,"phone").value;field(root,p,"email").value=x.email||field(root,p,"email").value;field(root,p,"cep").value=x.cep||field(root,p,"cep").value;field(root,p,"address").value=address||field(root,p,"address").value;U.toast("Dados do CNPJ preenchidos.")}catch(_){U.toast("CNPJ não encontrado ou serviço indisponível.","error")}finally{btn.disabled=false;btn.textContent="Buscar"}});
    root.querySelectorAll("[data-lookup-cep]").forEach(btn=>btn.onclick=async()=>{const p=btn.dataset.prefix||"",input=field(root,p,"cep"),cep=input.value.replace(/\D/g,"");if(cep.length!==8)return U.toast("Informe um CEP com 8 dígitos.","error");btn.disabled=true;btn.textContent="Buscando…";try{const r=await fetch(`https://viacep.com.br/ws/${cep}/json/`),x=await r.json();if(x.erro)throw new Error();field(root,p,"address").value=[x.logradouro,x.complemento,x.bairro,x.localidade,x.uf].filter(Boolean).join(", ");U.toast("Endereço preenchido pelo CEP.")}catch(_){U.toast("CEP não encontrado ou serviço indisponível.","error")}finally{btn.disabled=false;btn.textContent="Buscar"}});
  }
  function detail(id) {
    const c = Store.get("clients", id), orders = Store.all("orders").filter(o => o.clientId === id);
    if (!c) return;
    U.modal({ title: c.name, wide: true, body: `<div class="grid"><div class="card span-4"><p><b>Telefone</b><br>${U.esc(c.phone || "—")}</p><p><b>WhatsApp</b><br>${U.esc(c.whatsapp || "—")}</p><p><b>CPF / CNPJ</b><br>${U.esc(c.cpf || c.cnpj || "—")}</p><p><b>CEP</b><br>${U.esc(c.cep || "—")}</p><p><b>E-mail</b><br>${U.esc(c.email || "—")}</p><p><b>Endereço</b><br>${U.esc(c.address || "—")}</p><p><b>Observações</b><br>${U.esc(c.notes || "—")}</p></div><div class="card span-8"><div class="card-head"><h2>Histórico de ordens</h2><span class="badge">${orders.length}</span></div>${orders.length ? `<div class="table-wrap"><table><thead><tr><th>OS</th><th>Equipamento</th><th>Status</th><th>Data</th></tr></thead><tbody>${orders.map(o => `<tr data-open-order="${o.id}"><td><b>${o.number}</b></td><td>${U.esc(o.brand)} ${U.esc(o.model)}</td><td><span class="badge ${U.statusClass(o.status)}">${o.status}</span></td><td>${U.date(o.receivedAt)}</td></tr>`).join("")}</tbody></table></div>` : U.empty("Nenhuma ordem", "Este cliente ainda não possui atendimentos.")}</div></div>`, onOpen: root => root.querySelectorAll("[data-open-order]").forEach(x => x.onclick = () => window.Mello.Orders.detail(x.dataset.openOrder)) });
  }
  function remove(id) {
  ...
  function render() {
    ...
    const list = Store.all("clients").filter(c => [c.name,c.phone,c.whatsapp,c.cpf,c.cnpj,c.cep,c.email].join(" ").toLowerCase().includes(q));
  ...
  window.Mello.Clients = { render, edit, detail, remove, setupLookups };
})();
